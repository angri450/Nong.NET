...
暂保持 MIT。等 CLI 就绪后统一切换为 Apache 2.0，与 groundpa-toolkit skill 保持一致。原因：允许商用合作、专利保护条款、与依赖的开源项目协议兼容。
...
另有 3 个问题归属 skill 层（groundpa-toolkit），非包问题：Write 工具中文引号转换（#1）、dissect 脚本路由检测（#4）、word skill dispatch 优先级（#6）。
...
- [ ] groundpa-toolkit word skill 适配新包结构
- [ ] groundpa-toolkit genre skill 新建
- [ ] Docx 旧论文文件清理（6 个文件仍保留在 Docx/ 中，待测试通过后删除）
- [ ] 打包发版：Docx 3.1.0 + Inspect 3.0.0 + Genre 3.0.0

## 六、相关文件

- 仓库：`C:\Users\Administrator\Documents\Github\Angri450.Nong`
