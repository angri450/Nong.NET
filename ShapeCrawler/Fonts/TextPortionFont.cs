    ...
    A.Text aText) : ITextPortionFont
{
    ...
    public string EastAsianName
    {
        ...
        set => this.UpdateEastAsianName(value);
    }
    ...
    private void UpdateEastAsianName(string eastAsianFont)
    {
        ...
        var aEastAsianFont = aCurrentProperties.GetFirstChild<A.EastAsianFont>();

        if (aEastAsianFont is null)
        {
            aEastAsianFont = new A.EastAsianFont();
            aCurrentProperties.Append(aEastAsianFont); // to avoid ignoring color
        }
