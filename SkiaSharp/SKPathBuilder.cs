...
namespace SkiaSharp
{
	public unsafe class SKPathBuilder : SKObject, ISKSkipObjectRegistration
	{
		...
		public SKPathBuilder (SKPath path)
			: this (SkiaApi.sk_path_clone (path?.Handle ?? throw new ArgumentNullException (nameof (path))), true)
		{
			if (Handle == IntPtr.Zero) {
				throw new InvalidOperationException ("Unable to create a new SKPathBuilder instance.");
			}
		...
		{
			_detached = true;
			return SKPath.GetObject (Handle);
		}

		public SKPath Snapshot () =>
			SKPath.GetObject (SkiaApi.sk_path_clone (Handle));
		...
		public unsafe void ArcTo (SKPoint r, float xAxisRotate, SKPathArcSize largeArc, SKPathDirection sweep, SKPoint xy) =>
		...
		public void AddRoundRect (SKRoundRect rect, SKPathDirection direction = SKPathDirection.Clockwise)
		{
			...
			// Fallback: extract the rect and rx/ry from SKRoundRect
			// The native sk_path_add_rrect may not exist, use rounded rect parameters
		...
		public unsafe void AddPoly (ReadOnlySpan<SKPoint> points, bool close = true)
		{
		...
		public unsafe void AddPath (SKPath other, in SKMatrix matrix, SKPathAddMode mode = SKPathAddMode.Append)
		{
			if (other == null)
				throw new ArgumentNullException (nameof (other));
		...
		public void ReverseAddPath (SKPath other)
		{
			...
			// sk_path_reverse_add_path not available in native DLL.
			// Fallback: iterate points in reverse and add them manually.
			// ScottPlot's grid rendering does not call this, so a NotImplementedException
			// is acceptable for now; if needed, implement with path iteration.
			throw new NotImplementedException (
				"SKPathBuilder.ReverseAddPath is not supported on this SkiaSharp native version. " +
				"Use AddPath with a reversed path instead.");
		}
	}
}
