...
public class MetricsGetter
{
    ...
    private static XElement GetWmlMetrics(string fileName, WordprocessingDocument wDoc, MetricsGetterSettings settings)
    {
        ...
        foreach (var d in wxDoc.Descendants())
        {
            if (d.Name == W.saveThroughXslt)
            {
                ...
                var tempExternalRelationship =
                    wDoc.MainDocumentPart.DocumentSettingsPart.ExternalRelationships.FirstOrDefault(h => h.Id == rid);
                if (tempExternalRelationship is null)
                    ...
                    xDoc.Descendants(W.p)
                        .Select(p =>
                        {
                            ...
                            if (
                                lif is not null
                                && lif.IsListItem
                                && lif.Lvl(ListItemRetriever.GetParagraphLevel(p)) is not null
                            )
                            {
    ...
    )
    {
        ...
        var fontTypeArray = runText.Select(ch => FormattingAssembler.DetermineFontTypeFromCharacter(ch, csa)).ToArray();
